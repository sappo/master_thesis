...
Schritte grob in vier Phasen gliedern. Zunächst die Vorverarbeitung, um
...
* **Parser**. Der Parser liest Datensätze aus einer Datenquelle in eine Menge
  von Tupel, wobei jedes Tupel einem Datensatz entspricht.
...
\dots, (f_m, sim)}, wobei $m$ die Anzahl der Attribute pro Datensatztupel sind
und $sim = {sim_1, \dots, sim_r}$ eine von $r$ möglichen Ähnlichkeitsfunktionen,
die durch die Engine bereitgestellt werden. Das Klassifikationsmodell $M$ ist
spezifisch für den eingesetzten Klassifikator. Die genau Definition ist für die
Funktionsweise und dem Verständnis der Engine irrelvant.

In @fig:engine sind die Komponenten für die Fit-Phase gelb hervorgehoben. Bei
...
zu messen und die Auszuwerten. Diese liefern ein wichtiges Indiz, wie gut
Komponente. Des Weiteren ist es dadurch möglich das Zusammenspiel der
Komponenten untereinander zu bewerten, indem beispielsweise eine alternative
...
Operationen einer Komponente benötigen. Beispielsweise wird gemessen, wie
...
werden für ein Entity Resolution Workflow hochqualitative Blöcke erzeugt. Für
dynamische Verfahren, die Anfragen im Subsekundenbereich beantworten und
möglichst gleiche Latenzen versuchen zu erreichen, ist aufgrund der niedrigen
Dichte von Duplikaten in Datensätzen, die Fisher-Score als Bewertungskriterum
ungeeignet. Zwar werden für die Duplikate Blöcke generiert, die es erlauben
möglichst alle zur Anfrage passenden Enitäten schnell und präzise zu erhalten,
allerdings ist der Großteil aller Anfragen ergebnislos. Ergebnislos in diesem
Zusammenhang bedeutet, dass zu einer Anfrage kein Match, als keinen Datensatz
gibt, der der selben Entität entspricht. Für diese Datensätze trifft die
Fisher-Score keine Aussage, weshalb die Blöcke, die generiert werden, in welchen
sich keine Matches befinden, zum Teil sehr groß werden können. Aufgrund der
Menge dieser Anfragen, sinkt die Effektivtät des ER Systems dadurch dramatisch
reduziert.
...
Berücksichtigung der erzeugten Blöcke. Dazu werden wie bei Kejriwal & Miranker
Ausdrücke gebildet. Diese werden neben der Qualität aber auch in ihrer
Effektivität untersucht. Die Evaluierung eines Ausdrucks ist in Algorithmus
\ref{alg:dnf_eval} beschrieben. Zur Bewertung eines Ausdrucks $t$ benötigt der
Algorithmus den Datensatz $D$, die Matches $P$, sowie die Non-Matches $N$, der
Ground Truth des Label Generators. Zudem wird der von der Engine eingesetzen
Indexer $IX$ benötigt. Der Ausdruck $t$ wird dem Indexer $IX$ als Blocking
Schema übergeben, wodurch dieser in der Lage ist seinen Index $I$ über $D$ zu
bauen (Zeile 1). Dadruch kann der Block DNF Generator eine DNF erzeugen, die auf
den Indexer zugeschnitten ist. Hierzu werden alle generierten Blöcke aus $I$
einzeln betrachtet (Zeile 4). Für jeden Block werden zunächst die
Paarkombinationen aller dem Block zugehöriger Datensätze ermittelt. Diese werden
der Menge aller Paarkombinationen aller Blöcke $C$ hinzugefügt (Zeile 5). Zudem
wird, mittels der Matches $P$ und Non-Matches $N$, die Anzahl der true
positives, false positives und true negatives ermittelt und Global in $TP$, $FP$
und $FN$ aufsummiert (Zeile 6). Anhand von $TP$, $FP$, $FN$ wird das F-measure
zur Bewertung des Ausdrucks $t$ bestimmt. Bei der Disjunktion von Ausdrücken
kann dieses F-measure lediglich zur Vorauswahl der infragekommenden Audrücke
genutzt werden, da sich die F-measure Werte verschiedener Ausdrücke aus
unterschiedlichen Paarkombinationen berechnen. Damit die Ausdrücke einfach
dijunktiert werden können, wird die Paarkombination auf die Ground Truth
abgebildet. Dazu werden zwei Arrays $y_{true}$ und ${y_pred}$ mit der Länge $|P
\cup N|$ erzeugt (Zeilen 9-10). Diese können beim Zusammenfügen der DNF einfach
verodert und daraus das F-measure bestimmt werden. Für die Abbildung auf die
Ground Truth müssen alles Matches $P$ (Zeile 11) und alle Non-Matches (Zeile 20)
betrachtet werden. $y_{true}$ gibt an, welcher Klasse ein Paar $p$ angehört:
Match, wenn $p \in P$ (Zeile 13) oder Non-Match, wenn $p \in N$ (Zeile 22).
$y_{pred}$ bestimmt, ob ein Datensatzpaar einen gemeinsamen Block in $I$ hat
$y_{pred}[p] = True$ (Zeilen 15,24) oder nicht $y_{pred}[p] = False$ (Zeilen
17,26). Die Werte für F-measure, $y_{true}$ und $y_{pred}$ werden zum Schluss an
...
Disjunktion potentiell unendlich groß ist, wird diese über die Parameter $k$ für
die Konjunktionen und $d$ für die Disjunktionen begrenzt. Laut Kejriwal &
Miranker [@KM:Unsupervised:13] bieten Werte $>3$ keine wesentliche Verbesserung.
Ein weiterer Grund die Konjunktionen bzw. Disjunktionen nicht beliebig zu
erhöhen ist, dass der Indexer deutlich komplexere Blockschlüssel erzeugen muss,
was sich in der Effektiviät niederschlägt. Der Algorithmus beginnt die Ausdrücke
für die DNF zu erzeugen, indem alle Kombinationen der Menge spezifischer
Blockingprädikte $S$ bis zur Länge $k$ der maximalen Konjunktionen berechnet
werden (Zeilen 3-5). Anschließend wird jeder Ausdruck $t$ durch Algorithmus
\ref{dnf:eval} evaluiert. Ist der F-measure Wert, der für $t$ bestimmt wurde,
gleich 0, indiziert dies einen ungeeigneten Block und der Ausdruck wird entfernt
(Zeile 9). Aus den noch in $T$ vorhandenen Ausdrücken werden durch Disjunktion
bis zur Länge $d$ mögliche Kandidaten eines Blocking Schemas generiert (Zeilen
12-14). Ein Blocking Schema wird ausgewählt, indem die $y_{pred}$ und $y_{true}$
Arrays der Ausdrücke in jedem potentiellen Blocking Schema verodert werden
(Zeilen 19-22) und daraus das F-measure berechnet wird (Zeile 23). Das
potentielle Blocking Schema mit dem höchsten F-measure wird abschließend
ausgewählt und an die Engine zurückgegeben.
...
```texalgo
#alg:mdysimIII_insert DNF Similarity-Aware Index - Build
\Require
\Statex{
  \begin{itemize}[noitemsep, topsep=0pt, leftmargin=*, label={-}]
  \item Data set: $D$
