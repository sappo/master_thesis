...
$t$ benötigt der Algorithmus den Datensatz $D$, die Matches $P$, sowie die
Non-Matches $N$, der Ground Truth. Zudem wird, der von ein Engine
instanziierter, Indexer $IX$ benötigt. Der Ausdruck $t$ wird dem Indexer $IX$
...
die Konjunktionen und $d$ für die Disjunktionen begrenzt. Ein weiterer Grund die
Konjunktionen bzw. Disjunktionen nicht beliebig zu erhöhen ist, dass der Indexer
deutlich komplexere Blockschlüssel erzeugen muss. Bei $l$ muss der Indexer $l$
Funktionen, der spezifischen Blockprädikate, aufrufen, damit darauf der
Blockschlüssel zusammengefügt werden kann, was sich in der Effektiviät
niederschlägt. Der Algorithmus beginnt die Ausdrücke für die DNF zu erzeugen,
indem alle Kombinationen der Menge spezifischer Blockingprädikte $S$ bis zur
Länge $k$ der maximalen Konjunktionen berechnet werden (Zeilen
\ref{alg:bs:dis}-\ref{alg:bs:die}). Anschließend wird jeder Ausdruck $t$ durch
oben erklärten Algorithmus \ref{dnf:eval} evaluiert. Ist der F-measure Wert $f$,
der für $t$ bestimmt kleiner einer Schranke $thres$, indiziert dies einen
ungeeigneten Block und der Ausdruck wird entfernt (Zeile \ref{alg:bs:del}). Aus
den noch in $T$ vorhandenen Ausdrücken werden durch Disjunktion bis zur Länge
$d$ mögliche Kandidaten eines Blocking Schemas generiert (Zeilen
\ref{alg:ref:cms}-\ref{alg:ref:cme}). Ein Blocking Schema wird ausgewählt, indem
die $y_{pred}$ und $y_{true}$ Arrays der Ausdrücke in jedem potentiellen
Blocking Schema verodert werden (Zeilen \ref{alg:bs:ort}-\ref{alg:bs:ory}) und
daraus das F-measure berechnet wird (Zeile \ref{alg:bs:fm}). Das potentielle Blocking
