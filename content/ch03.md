...
deutlich größerer, anweden lassen, wird in @sec:free_params evaluiert.
...
spezifische Blockingprädikate $P_1, P_2, P_3$, eine Relation $S(p, r)$, die
anhand eines Prädikates $p$ und eines Datensatzes $r$ eine Menge von
Blockschlüsseln erzeugt, sowie eine Verknüpfung $\circ$, die die Mengen der
Prädikate verknüpft. Betrachtet man die Konjunktion strikt, so gilt für $\circ$
die Distributivität und die Kommutativität. Die Distributivität ist für die
weitere Betrachtung nicht interessant, da deren Auswirkung, bei der Generierung
von Schlüsseln, über die Festlegung einer Reihenfolge, beispielsweise $S(P_2, r)
\circ S(P_1, r) \circ S(P_3, r)$, erzwungen werden kann. Interessant hingegen
ist die Kommutativität, da diese die Blockbildung eines Blocking Verfahrens
maßgeblich beeinflussen kann. Hierzu wird der Ausdruck $A
...
vertauschter Attribute, einem gemeinsamen Block zugeordnet. Das Gleiche
gilt aber auch für das Paar (1, 3), sodass `Peter` $\circ$ `Moritz` $\circ$
`Michel`$=$ `Michel` $\circ$ `Moritz` $\circ$ `Peter` und dieses Paar ebenfalls
einem gemeinsamen Block zugeordnet wird. Für den Fall, dass $\circ$ nicht
...
auf Kosten der Pairs Completeness verbessert. Die Kommutativität kann jedoch
keinen Einfluss auf Rechtschreibfehler, wie im Falle von Paar (3, 4) nehmen.
Eine Möglichkeit Rechtschreibfehler bei der Erzeugung der Blockschlüssel
...
transpositive Rechtschreibfehler zu filtern, muss die Kommutativität auf
Zeichenebene erweitert werden. Beispielsweise indem die Teilschlüssel
konkateniert und als Anagramme behandelt werden. Dadurch ist es wiederum möglich
die Pairs Completeness zu erhöhen. Allerdings ebenfalls auf Kosten der Pairs
Quality, da die Kollisionsrate durch Reduktion der möglichen Blockschlüssel
erhöht wird.
