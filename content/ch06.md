...
* Bei der Kreuzvalidierung sollte der Datensatz bestmöglich repräsentiert sein.
...
* Ein Qualitätsmaß muss sowohl Recall als auch Precision berücksichtigen,
  ...
  eine Baseline, bei welcher die Kandidatenmenge $C$ gleich Ergebnismenge $R$
...
zwischen den Paaren in der Kandidatenmenge $C$. Die folgenden vier
Konfigurationen werden hierzu evaluiert:
...
Teilvektoren sind dabei identisch. Es gibt zum einen, Punkte in der linken
oberen Ecke und der rechten unteren, wobei ein Punkt einem Ähnlichkeitswert
entspricht. Haben mehrere Paare denselben Ähnlichkeitswert, werden diese in
einem Punkt zusammengefasst. Dementsprechend gibt es Paare mit sehr niedriger
Ähnlichkeit (links oben) und zum anderen Paare mit sehr hoher Ähnlichkeit
(rechts unten), allerdings nichts dazwischen. Dementsprechend hat der
Klassifikator fast ausschließlich nur ein Attribut und einen binären
Ähnlichkeitswert, um eine Entscheidung zu treffen, was die schlechten Werte aus
@sec:fp_fusion erklärt. Im Gegensatz dazu bietet der Vollvektor dem
Klassifikator deutlich mehr Unterscheidungsmerkmale für die Paare. Dies lässt
sich in den Precision-Recall Kurven daran erkennen, dass diese vollständig mit
Punkten besetzt ist. Der Klassifikator, der mit den Vollbesetzen Vektoren am
besten abschneidet, ist die SVM mit Linearkernel. Dennoch verliert selbst dieser
mit 73 % einen großen Anteil an Recall, gegenüber 95 % bei der Baseline. Dafür
ist die Precsion mit 55 % deutlich besser als die 12 % der Baseline.
...
mit vorausberechneten Ähnlichkeiten erzielt werden. Bei der Baseline mit voller
...
mit voller Ähnlichkeitsberechnung genutzt, trotz der schlechteren Effizienz.

Beim Auswerten der Daten gab es noch eine Unstimmigkeit. Und zwar wurden als
Ähnlichkeitsmaß für alle Attribute die Bag-Distanz ausgewählt. Ein genauer Blick
...
Bezug auf den MDySimIII mit Klassifikator und voller Ähnlichkeitsberechnung,
analysiert.
...
ausgewählte Ähnlichkeitsmaß deutlichen Einfluss auf die Qualität des
Klassifikator hat.
