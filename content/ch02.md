...
Als Entity Resolution (ER) wird das Suchen und Finden von gleichen Enitäten in
einer oder mehreren Datenquellen bezeichnet. In der klassischen Variante
arbeitet Entity Resolution auf statischen Daten. D.h. das während des
ER-Prozesses keine neuen Daten hinzukommen. Hierbei werden die zwei Disziplinnen
Deduplizierung und Entity-Linking unterschieden. Die Deduplizierung wird auf
einer Datenquelle durchgeführt und hat den Zweck alle Duplikate in dieser
Datenquelle zu finden. Anschließend werden die gefundenen Duplikate automatisch
oder manuell zusammengeführt. Entity Linking hingegen wird auf mindestens zwei
verschiedenen Datenquellen durchgeführt. Das Ziel ist es, nicht Duplikate
zusammenzuführen, sonderen Entitäten unter den Datenquellen zu verlinken. Damit
die Links eindeutig sind, wird vorausgesetzt, dass die einzelnen Datenquellen
dedupliziert sind.

Im Gegensatz zum statischen ER gibt es das dynamische ER. Der Ausgangspunkt für
diese Disziplin ist eine deduplizierte Datenquelle. Das finden gleicher Enitäten
erfolgt hierbei auf Anfrage, weshalb die gesamte Datenmenge vorab nicht bekannt
ist. Ein dynamisches ER System ist ähnlich einer Suchmaschine, doch anstatt
einer gewerteten Liste möglicher Treffer, soll es alle gleichen Entitäten
finden, welche zur Anfrage passen. Das bedeutet insbesondere, dass die Anfrage
die gleiche Datenstruktur haben muss, wie die zu durchsuchende Datenquelle.
Zudem ergeben sich Anforderungen an die Antwortzeit, welche bei vielen System im
Subsekundenbereich liegt.


Prinzipelle ist Entity Resolution ein Problem das quatratische Komplexität
aufweist, da jeder Datensatz mit jedem anderen Verglichen werden muss, um alle
Duplikate zu finden. Allerdings führt dies bei großen Datenmengen dazu, dass die
Ausführungszeit im statischen Fall bzw. die Latenzen im dynamischen Fall
unakzeptabel groß werden.
...
sollen sich mögliche Duplikate in der "Nähe" von einander einfinden. Zur
...
nicht einzigartig sein. Um die Berechnung des Schlüssels gering zu halten, soll
...
Priorität. In der zweiten Phase werden die Datensätze anhand des Schlüssels
...
gewählten Fenstergröße abhängen. Ein großer Nachteil ist, dass Datensätze die
sich in der ersten Stelle des Schlüssels unterscheiden, weit voneinander
entfernt sind und dadurch nicht als Matches identifiziert werden. Um dennoch eine
...
Die Idee von Canopy Clustering ist es, Datensätze anhan einer einfachen
...
- 1$ Vergleiche notwendig. Da dies zu ungewollt hohen Latzen führen würde,
...
Ähnlichkeiten vorzuberechnen, um während der Laufzeit diese einfach nur
nachschlagen zu müssen.

Der Index besteht aus drei Teilen. Dem **Record Index**, welcher alle Attribute
speichert und diese ihren Datensätzen zuordnet. Der **Block Index**, welcher
Attribute anhand einer Enkodierungsfunktion gruppiert. Und zuletzt der
**Similarity Index**, welcher die selben Schlüssel wie der Record Index
verwendet und die Ähnlichkeiten der Attribute im gleichen Block hält.

Das Einfügen von Datensätzten läuft nach folgendem Schema ab. Zunächst werden
alle Attribute mit Verweis auf den Datensatzidentifier im Record Index
gespeichert. Falls ein Attribute dort schon existiert wird lediglich der
Identifier angefügt. Anschließend wird für jedes Attribute eine Enkodierung
bestimmt. Anhand dieser Enkodierung werden die Attribute in jeweils einen Block
im Block Index eingefügt. Beinhaltet der Block mehr als ein Attribute wird zu
allen Attributen die Ähnlichkeit bestimmt und die Ähnlichkeit des eingefügten
mit der Verglichenden in den Similarity Index eingefügt. Gleichzeitig wird die
bestimmte Ähnlichkeit des eingefügten Attributes auch zu den Verglichenden im
Similarity Index ergänzt.
...
gleiches Attribute besitzen und werden in einen Accumulator mit dem
...
Similarity Index im Accumulator aufsummiert.
...
muss und lediglich Werte nachgeschlagen werden. Auf der negative Seite steht
...
Attribute untschiedlicher Datensatze $t_1$ und $t_2$ und nutzt die $i^te$
...
und $t_2$ und wendet $p_i$ auf die ensprechenden Attribute $f_1$ und $f_2$ der
...
Erstellen ist ein sehr aufwändiger Prozess, welcher von einem Domainexperten
