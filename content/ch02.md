...
= \{r_i|r_i.eid = q_j.eid, r_i \in R\}, M_{q_j} \subseteq R, q_j \in Q,$$ wobei
$eid$ ein eindeutiger Identitiver einer Entität ist, welcher so nicht existiert.
Die Herausforderung für dynamische ER-Verfahren ist weiter nach Ramadan et al.
Indexing-Verfahren zu entwickeln, welche es erlauben den Index dynamische zu
...
Systems, sind die Ereignisse Anfragen. Dabei besteht eine Anfrage aus einem
Datensatz, der strukturell identisch zu den Bestandsdaten ist. Das Ergebnis
einer Anfrage ist eine Menge von Datensätzen, die derselben Entität entsprechen.
Bei vielen Onlinediensten müssen zu einer Anfrage, in nahe Echtzeit, dieselben
Entitäten im eigenen Datenbestand gefunden werden. Anwendungsgebiete sind etwa
Produktsuchen, Kreditauskünfte oder die Suche nach Straftäter in einer
Polizeidatenbank. So unterschiedlich diese Domänen auch sind, ein Entity
Resolution Framework muss für alle dieselben drei Probleme lösen:

* **Niedrige Latenzen**. Der Zeitraum vom Stellen einer Anfrage bis zu deren
  Beantwortung sollte und eventuell muss im Subsekundenbereich liegen. Die
  Gründe dafür sind vielfältig, beispielsweise bietet eine Produktsuche eine
  bessere Benutzererfahrung, je schneller die Daten verfügbar sind, zudem sind
  Benutzer dazu geneigt einen Dienst nicht mehr zu nutzen, wenn die
  Benutzererfahrung durch zu lange Wartezeiten getrübt wird. Eine Kreditauskunft
  ist meist Teil eines größeren Prozesses, sodass deren Laufzeit die
  Benutzererfahrung von anderen Diensten beeinflussen kann, weshalb eine solche
  Auskunft seinen Kunden Garantien für die Laufzeit der Anfragen gibt, um sich
  gegenüber der Konkurrenz hervorzuheben. Bei der Suche nach Straftätern kann
  die Laufzeit beeinflussen, wie schnell ein Täter gefasst werden kann.
  Hauptverantwortlich für niedrige Latenzen sind in einem Entity Resolution
  Framework die Blocking Verfahren. Während ist die Berechnung der
  Ähnlichkeitswerte zwischen Attributen des Anfragedatensatzes und der
  Datensatzkandidaten, die derselben Entität entsprechen können, am längsten. Um
  niedrige Latenzen zu erreichen, muss das Blocking zum einen die zu
  durchsuchende Kandidatenmenge reduzieren und kann versuchen den Aufwand von
...
* **Datenmodifikation zur Laufzeit**. Der Datenbestand kann mit jedem Ereignis
  ...
  modifiziert wurde, muss diese für die darauffolgenden Anfragen zum Abgleich
  auffindbar sein. Werden die Anfragen stets sequentiell bearbeitet, ist diese
  Anforderung realtiv einfach zu erfüllen. Da aber auch bei hoher Last die
  Latenzen niedrig bleiben müssen, wird ein Entity Resolution Framework die
  Bearbeitung auf mehrere Threads, Prozesse oder Knoten skalieren, um die
  Anfragen parallel zu bearbeiten. Erhält das System nacheinander mehrere
  ...
  allerdings die zweite Anfrage die erste als Teil ihrer Ergebnissemenge erhält,
  falls diese eingefügt wurde, müssen diese sequentialisiert werden, ohne
  dadurch einen Flaschenhals für das Gesamtsystem zu bilden.
* **Hohe Trefferrate**. Unter den Latzen darf naturlich nicht die Qualität des
  Ergebnisses leiden. Das Ziel ist es zu einer Entität, alle im Datenbestand
  existierende Entität zu finden und zurückzugeben. Damit die Entitäten gefunden
  werden können, die während des Blockings erzeugt Kandidatenmenge auch alle
  Duplikate enthalten. Da diese Menge auch Datensätze enthält die nicht
  derselben Entität entsprechen müssen diese nach der Ähnlichkeitsberechnung,
  von einem Klassifikator, in Matches und Non-Matches. Damit dieser keine
  Missklassifikationen unternimmt, d.h. Matches als Non-Matches deutet bzw.
  umgekehrt. Muss der Ähnlichkeitswert eine hohe Aussagekraft haben. Deshalb ist
  es wichitg für jedes Attribut eine geeignete Ähnlichkeitsfunktion zu finden,
  die dem Klassifikator erlaubt möglichst eindeutige Entscheidungen zu treffen.
