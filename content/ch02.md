...
Ähnlichkeiten können nach Elmagarmid et al. [@EIV:Duplicate:07] auf
Attributsebene oder Datensatzebene erhoben werden.

### Attributsähnlichkeiten

Die Attributsähnlichkeiten kann man grob in fünf Kategorien ordnen:
...
Komplexität $O(|\sigma_1|*|\sigma_2|)$. Needleman und Wunsch [@?] erweitern die
originale Editierdistanz dahingehen, dass bestimmte Operationen anders gewichtet
werden. Dazu können die Kosten für die einzelnen Schritte, welche in der
einfachsten Form 1 sind, auf einen beliebigen Gleitkommawert angepasst werden.
In dieser Variante enspricht das Lösen der Editierdistanz dem Traveling Salesmen
Problem und ist daher NP-schwer.

Eine weitere Modifikation der Editierdistanz ist es etwa Lücken zu erkennen
[@AffineGapDistance], beispielsweise wenn ein Wort abgekürzt wurde.
...
geringeren Kosten zu versehen [@SmithWatermann].
...
berechnet sich die Jaro-Distanz $d_j$ folgendermaßen:

$d_j = \left\{\begin{array}{l l} 0 & \text{wenn }m = 0\\
...
$\sigma_1 = (\sigma_{1_{t_1}}, ..., \sigma_{1_{t_i}})$ mit allen Token aus
$\sigma_2 = (\sigma_{2_{t_1}}, ..., \sigma_{2_{t_j}})$ vergleichen. Zum
...
$s_{t_k} = max(sim(\sigma_{1_{t_k}}, \sigma_{2_{t_1}}), ...,
sim(\sigma_{1_{t_k}}, \sigma_{2_{t_j}}))$ berechnet. Die Gesamtähnlichkeit ist
der Durchschnitt der Tokenähnlichkeiten $m = avg(s_{t_1}, ..., s_{t_i})$. Das
...
$\sigma_1$, $\sigma_2$ ist dementsprechend definiert als

$$sim(\sigma_1, \sigma_2) = \frac{\sum_{i=1}^{|D|}{\nu_{\sigma_1}(j) \cdot
\nu_{\sigma_2}(j)}}{\|\nu_{\sigma_1}\| \|\nu_{\sigma_2}\|}$$

Mit WHIRL ist es möglich vertauschungsicher die Ähnlichkeit von Strings zu
bestimmen. Ein großer Vorteil dieses Algorithmus ist, dass nach erheben der
TF/IDF Statistik, die Ähnlichkeit in $O(1)$ berechnet werden kann, da lediglich
Werte nachgeschlagen werden müssen. Dem entgegen steht, wie bei Monge und Elkan,
dass typographische Fehler nicht erkannt werden. Deshalb erweitern Gravano et.
al [@WHIRL_QGRAM] WHIRL und nutzen statt atomare Strings Q-Gramme. Dadurch
lassen sich bei gleicher Komplexität auch Rechtschreibfehler erkennen, da ein
Großteil der Q-Gramme gleich ist. Allerdings geht zu zugunsten von
Speicherkosten, da die TF/IDF Statistik dementsprechend größer wird.
...
für Vektoren in $\mathbb{R}^d$ eindeutig über die Kosinus-Ähnlichkeit
$\|\sigma_1\|-\|\sigma_2\|$ berechnet werden [@SRR:Large:07]. Kernelfunktionen
sind die Hauptidee von Support Vector Machines (SVM), welche von Boser et al.
[@BGV:Training:92] eingeführt wurden. In ihrer einfachsten Form lernen SVMs
eine separierende Hyperplane zwischen einer Menge von Punkten, welche den
Abstand zwischen der Hyperplane und dem nähesten Punkt maximiert. Eine
Kernelfunktion berechnet das innere Produkt zwischen Objekten im Hyperraum
(Feature Space). Dazu bildet der Kernel die Objekte implizit auf den Feature
Space (Vektoren) ab [@LSS.EA:Text:02]. Diese Kernel weisen eine Reihe
statistischer interessanter Eigenschaften auf, sodass die Kernelfunktion von
Lodhi et al. [@LSS.EA:Text:02] generalisiert wurde.
...
bekannten Strings, erzeugt. Alternativen nutzen statt der Anzahl der Wörter
TF/IDF Gewichte. Wie bereits bekannt sind atomare String anfällig für
...
Space miteinander zu vergleichen. Der sogennante String Subsequence Kernel
(SSK) vergleicht Strings, indem er Stringvektoren erzeugt, welche einen
bestimmten Substring beinhalten oder nicht. Dabei wird jedes Vorkommen eines
Substrings anhand der Übereinstimmung gewichtet. Die Übereinstimmung erlaubt
beispielseweise auch Lücken, sodass der Substring 'c-a-r' in den beiden Wörtern
'**car**d' und ' **c**ust**ar**d' mit unterschiedlicher Gewichtung vorkommt.

### Datensatzähnlichkeiten

## Klassifizierer

Nachdem die Ähnlichkeit zwischen den Kandidatenpaaren berechnet wurden, gibt es
zu jedem Attributspaar $a_1, ..., a_n$ einen Vektor $(a_{1_s}, ..., a_{n_s})$
mit der ermittelten Attributsähnlichkeit $s$. Nun ist es die Aufgabe der
Klassifizierer, anhand dieser Vektoren, die Menge der Kandidatenpaare in Match
und None-Matches zu trennen. Bei den Klassifizieren kann man grundsätzlich
unterscheiden zwischen benutzerdefinierten und gelernten Strategien.

#### Benutzerdefiniert

* **Schwellenwertbasierend**. Hierbei werden zunächst die Vektoren zu einer
  Gesamtähnlichkeit $g$ aufsummiert oder gemittelt. Anschließend werden je nach
  Ausprägung bis zu zwei Schwellen festgelegt. In der Variante mit einer
  Schwelle $t$, werden alle Kandidatenpaare mit $g > t$ als Matches und $g \leq
  t$ als None-Matches klassifiziert. Werden zwei Schranken $t1$ und $t2$
  genutzt, werden Matches mit $g > t1$ und None-Matches mit $g \leq t2$
  klassifiziert. Zusätzlich gibt es noch den Bereich $t2 < g \leq t1$, welcher
  unsicher bedeutet und manuelle klassifiziert werden muss.
* **Regelbasierend**. Idee ist es, eine Prädikatenlogik erste Stufe (PL1) auf
  die Vektoren anzuweden.
