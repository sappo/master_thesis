...
## Blocking Schema nach [@KM:Unsupervised:13]
...
Um dieses Problem zu lösen schlage Kejriwal & Mirankern [@KM:Unsupervised:13]
...
müssen. Ist $sim \geq ut$ wird das Paar positiv klassifiziert. Ebenso, ist
$sim < lt$ wird ein Paar negativ klassifiziert. Damit die Menge der
klassifizierten Daten nicht beliebig groß wird, kann der Benutzer festlegen, wie
viele positive bzw. negative Paare maximal erzeugt werden sollen. Dabei werden
sowohl bei den positiven als auch bei den negativen Paaren die besten $d$ bzw.
$nd$ Paare ausgewählt. Bei den negativen Paaren, soll dadurch verhindern, dass
lediglich Paare mit $sim \approx 0.0$ ausgewählt werden, da diese für gewöhnlich
zu niedrigen Klassifikationsraten führen. Die Gesamtkomplexität des Algorithmus
ist $O(n + nm + nm)$. Die Bestandteile sind $O(n)$ zur Erzeugung der TF/IDF
